...
#' @title  Get all fertilizer information for associated with factors in the experimental design
#' @description retrieve fertilizer information from AGROFIMS database. The fertilizer information includes fertilizer products and nutrients elements. 
...
#' @examples \dontrun{
#' sc <- AgExpDesign$new(serverURL = "https://research.cip.cgiar.org/agrofims/api/dev", version = "/0233/r")
#' out <- sc$ag_get_edsfert_expsiteId(expsiteDbId = 5,
#'                                       format = "data.frame")
#' out <- sc$ag_get_edsfert_expsiteId(expsiteDbId = 6,
#'                                       format = "data.frame")
#' }
#' @export 
#' 
AgExpDesign$set(which = "public", name = "ag_get_edsfert_expsiteId", 
                    function( expsiteDbId =NULL,
                              format=c("json","list","data.frame"),
                              ... ){  
                      
                      super$endPoint <- "/site-factor-fertilizer/getAll?id="
                      url  <- paste0(self$serverURL, self$version, super$endPoint) #everything before the URL
                      print(url)
