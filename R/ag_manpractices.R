...
#' 
#' Management Measurements Class
#' 
#' @description Class to represent Management measurements data from AgroFIMS. It inherents from AgAPI client class 
...
AgManPrac <- R6::R6Class(
  ...
  public = list(
    ...
    ag_get_manprac_sitecropId = function(siteCropDbId, format, ...){} ,
    ag_get_manprac_expsiteId = function(expsiteDbId, format, ...){}
...
#' @title  Get all Management measurements based on the experiment site ID
#' @description retrieve Management measurements data from AGROFIMS database. AGROFIMS use the AgAPI standard in order to get and post data.
#' @field ... argument inherents by AgaAPIClient
#' @field siteCropDbId character siteCropId its the id of the Management evaluated in one site and experiment
...
#' @export 
#' 
AgManPrac$set(which = "public", name = "ag_get_manprac_sitecropId", 
               function( siteCropDbId =NULL,
                         format=c("json","list","data.frame"),
                         ... ){  
                 
                 super$endPoint <- "/crop_man_practices/getAll?id="
...
#' @title  Get all Management measurements based on the agronomic experiment site database ID
#' @description retrieve Management measurements data from AGROFIMS database with AgAPI standard
#' @field ... argument inherents by AgaAPIClient
...
#' @examples \dontrun{
#' library(ragapi)
#' .x <- AgManPrac$new(serverURL="https://research.cip.cgiar.org/agrofims/api/dev", version="/0253/r")
#' out <- .x$ag_get_manprac_expsiteId(expsiteDbId= 11, format="data.frame")
#' }
#' @export 
#' 
AgManPrac$set(which = "public", name = "ag_get_manprac_expsiteId", 
               function(expsiteDbId =NULL,
                 ...
                 # print(sitecrop_id)
                 # 
                 # super$endPoint <- "/crop-Management/getAll?id="
                 # url  <- paste0(self$serverURL, self$version, super$endPoint) #everything before the URL
                 # print(url)
                 # 
                 # #GET parameters for retrieving data ----------------------------
                 ...
                 # 
                 # #Iterate over exp_site_id to extract Management information by site from each site in the study
