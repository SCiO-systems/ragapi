...
#' Personnel Class
#' 
#' @description subclass for representing the personnel data. It inherents from AgAPI client class 
...
#' @export
#' 
AgPersonnel <- R6::R6Class(
  
  classname = "AgPersonnel",
  inherit = AgAPIClient,
  ...
  public = list(
    ...
    ag_get_personnel_studyId = function(studyDbId, format,...){}
  )
)

#' @title  Get all personnel project entities info from an agronomic study by its agronomical study database ID
...
#' @export 
#' 
AgPersonnel$set(which = "public", name = "ag_get_personnel_studyId",  function(studyDbId =NULL,
                                                                     format=c("json","list","data.frame"),...){
  
  format <- match.arg(format)
  super$endPoint <- "/personnel/getAll?id="
